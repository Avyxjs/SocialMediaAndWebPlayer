$(function () {
    ...
    // Handle volume bar click events
    $('#volume-bar-box').click(function(event) {
      // Calculate the new volume based on where the user clicked
      var boxOffset = $(this).offset().left;
      var boxWidth = $(this).width();
      var clickX = event.pageX - boxOffset;
      var newVolume = Math.round((clickX / boxWidth) * 100);
      setVolume(newVolume/100);
      
      // Update the volume bar and icon
      $('#volume-bar').width(newVolume + '%');
      if (newVolume === 0) {
        $('#volume-icon i').removeClass('fa-volume-up').addClass('fa-volume-mute');
      } else {
        $('#volume-icon i').removeClass('fa-volume-mute').addClass('fa-volume-up');
      }
    });
    ...
    function selectTrack(flag) {
      ...
      if (currIndex > -1 && currIndex < albumArtworks.length) {
        ...
        currAlbum = title[currIndex];
        currTrackName = artist[currIndex];
